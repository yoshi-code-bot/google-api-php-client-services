...
class Google_Service_Classroom_Resource_CoursesCourseWorkMaterials extends Google_Service_Resource
{
   ...
   * requested course, create course work material in the requested course, share
   * a Drive attachment, or for access errors. * `INVALID_ARGUMENT` if the request
   * is malformed or if more than 20 * materials are provided. * `NOT_FOUND` if
   ...
   * the corresponding course work material item.
   *
   * This method returns the following error codes:
   *
   * * `PERMISSION_DENIED` if the requesting developer project did not create the
   * corresponding course work material, if the requesting user is not permitted
   ...
   * if no course exists with the requested ID. (courseWorkMaterials.delete)
   *
   * @param string $courseId Identifier of the course. This identifier can be
   * either the Classroom-assigned identifier or an alias.
   ...
   * the requested course work material has already been deleted. * `NOT_FOUND` if
   * the requested course or course work material does not exist
   * (courseWorkMaterials.patch)
   *
   * @param string $courseId Identifier of the course. This identifier can be
   * either the Classroom-assigned identifier or an alias.
   * @param string $id Identifier of the course work material.
