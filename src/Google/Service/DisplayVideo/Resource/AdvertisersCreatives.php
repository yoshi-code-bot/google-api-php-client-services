...
class Google_Service_DisplayVideo_Resource_AdvertisersCreatives extends Google_Service_Resource
{
   ...
   * - `exchangeReviewStatus`     - `dynamic`     - `creativeId` * The operator
   * must be `HAS (:)` for the following fields:     - `lineItemIds` * For
   * `entityStatus`, `minDuration`, `maxDuration`, and `dynamic` there may be at
   * most one restriction. * For `dimensions`, the value is in the form of
   ...
   * in order to search against multiple possible line item IDs. * There may be
   * multiple `creativeId` restrictions in order to search against multiple
   * possible creative IDs.
   *
   * Examples:
   *
